    ...
    <div className="animate-fade-in space-y-6">
      ...
      {stats?.lowStockCount > 0 && (
        <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm">
          ...
          <Link to="/dashboard/inventory" className="ml-auto btn-secondary text-xs py-1.5 px-3 text-amber-700 border-amber-300 hover:bg-amber-100 whitespace-nowrap">
            View alerts →
          </Link>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between p-5 border-b border-surface-100">
            ...
            <Link to="/dashboard/invoices" className="text-xs text-brand-600 hover:text-brand-700 font-medium">View all →</Link>
          </div>
          <div className="divide-y divide-surface-100">
            {recentInvoices?.length === 0 && (
              <p className="p-5 text-sm text-slate-400 text-center">No invoices yet. <Link to="/dashboard/invoices/create" className="text-brand-600">Create one →</Link></p>
            )}
        ...
        <div className="card">
          <div className="flex items-center justify-between p-5 border-b border-surface-100">
            ...
            <Link to="/dashboard/analytics" className="text-xs text-brand-600 hover:text-brand-700 font-medium">Analytics →</Link>
          </div>
          <div className="divide-y divide-surface-100">
            {recentSales?.length === 0 && (
              <p className="p-5 text-sm text-slate-400 text-center">No sales data yet.</p>
            )}
      ...
      <div className="card p-5">
        ...
        <div className="flex flex-wrap gap-3">
          <Link to="/dashboard/invoices/create" className="btn-primary text-sm">🧾 New Invoice</Link>
          <Link to="/dashboard/products" className="btn-secondary text-sm">📦 Add Product</Link>
          <Link to="/dashboard/inventory" className="btn-secondary text-sm">🔔 Stock Alerts</Link>
          <Link to="/dashboard/analytics" className="btn-secondary text-sm">📊 Analytics</Link>
        </div>
      </div>
    </div>
  );
